...
def ts_graph_line(df_daily, TARGET, title):
    ### Graph the daily behavior of the bitcoin closing price
    fig = go.Figure()
...
def violin_plot(df, TARGET, time):
    ### Graph the probability distribution of the bitcoin closing price
    fig = go.Figure()
...
def graph_outliers(df_daily, TARGET):
    ### Graph the outliers of the bitcoin closing price
    #Create traces
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_daily.index, y=df_daily[TARGET], mode="lines", name=TARGET))
...
def graph_correlation_features(df_features, lag):
    ### Graph the correlation matrix of the features of the bitcoin closing price
